...
# new_capture()/use_capture()/urlsafe_id() live in helper-capture.R.
...
test_that("comment and vote bodies are still JSON-encoded", {
  ...
  encode <- cap$last()$dots$encode
  expect_true(is.null(encode) || identical(encode, "json"))
...
test_that("print and summary methods are registered for dispatch", {
  ...
  for (m in c(
    "print.virustotal_response", "print.virustotal_file_report",
